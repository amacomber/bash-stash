...
basicToken="xxxxxxxxxxxxxxxxxxxxxx"
...
Please double-check the serial number and try again."
        ...
        lastUser=$(curl -X GET "$URL/JSSResource/computers/udid/$UDID" -H "accept: application/xml" -H "Authorization: Bearer $bearerToken"  | grep -e "<realname>" | awk -F "<realname>|</realname>" '{print $4}')
...
Are you sure you want to unmanage this device?"
        userChoice=$("$jamfHelper" -windowType "$windowtype" -title "$title" -description "$description" -button1 "$button1" -button2 "$button2" -defaultButton 2 -cancelButton 1 -timeout "$timeout")
...
Please double-check the serial number and try again."
        ...
        ### Verify the information is correct
        lastUser=$(curl -X GET "$URL/JSSResource/computers/serialnumber/$serialNumber" -H "accept: application/xml" -H "Authorization: Bearer $bearerToken"  | grep -e "<realname>" | awk -F "<realname>|</realname>" '{print $4}')
