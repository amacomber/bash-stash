...
SECURETOKEN_ADMIN_PASSWORD=$( echo "${6}" | base64 --decode )

# Check for the Admin user specified
if [ ! -d /Users/"$SECURETOKEN_ADMIN_USERNAME" ];
then
  jamf policy -event createLocalAdmin;
else
  echo ""$SECURETOKEN_ADMIN_USERNAME" is already present on the machine."
fi

# Check to see if Admin has a SecureToken and prompt user if the answer is no
LoggedInUser=$(stat -f%Su /dev/console)
userApproval=$(osascript -e '
display dialog "Please enter your Mac login password." default answer "" with title "JumpCloud Agent User Authentication" buttons {"Stop","Submit"} default button "Submit" with hidden answer giving up after 180
if button returned of result is "Submit" then
	set userApproval to the (text returned of the result)
else
	set userApproval to "Stop"
end if')
stCheck=$(sysadminctl interactive -secureTokenStatus "$SECURETOKEN_ADMIN_USERNAME")
response="Secure token Is ENABLED for user $SECURETOKEN_ADMIN_USERNAME"
if [ "$stCheck" != "$response" ];
then
sysadminctl -adminUser "$LoggedInUser" -adminPassword "$userApproval" -resetPasswordFor "$SECURETOKEN_ADMIN_USERNAME" -newPassword "$SECURETOKEN_ADMIN_PASSWORD"
sysadminctl -secureTokenOn "$SECURETOKEN_ADMIN_USERNAME" -password "$SECURETOKEN_ADMIN_PASSWORD" -adminUser "$LoggedInUser" -adminPassword "$userApproval"
else
sysadminctl -adminUser "$LoggedInUser" -adminPassword "$userApproval" -resetPasswordFor "$SECURETOKEN_ADMIN_USERNAME" -newPassword "$SECURETOKEN_ADMIN_PASSWORD"
echo "$stCheck"
fi
...
if [[ $MacOSMajorVersion -eq 10 && $MacOSMinorVersion -lt 13 ]]; then
  ...
  readInPasswordForUser() {
    while true; do
      ...
      echo 'User selected Stop'
			exit 99 ### User selected "Stop"
